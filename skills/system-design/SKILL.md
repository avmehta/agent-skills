---
name: system-design
description: "Practice system design interviews. Four modes: `mock` (Codex interviews and scores), `learn` (Codex demonstrates a staff-level interview; `--auto` runs both roles when explicitly requested), `postmortem` (diagnose a past interview), and `generate` (create a question and rubric). Use for system-design practice, mocks, postmortems, or question generation."
metadata:
  version: "0.3.0"
...
Invoke as `$system-design ...` in Codex. The rest of this document writes commands without a prefix.

## Commands

| `$1` | Description | Reference |
|---|---|---|
| `mock [problem]` | Codex = strict interviewer. Phases enforced, scored debrief at end. | [reference/mock.md](reference/mock.md) |
| `learn [problem] [--auto]` | User = interviewer, Codex = staff candidate. `--auto` runs both via sub-agents while user observes. | [reference/learn.md](reference/learn.md) |
...
`--diagram-style` controls how Codex renders diagrams in modes that draw them (`learn`, `postmortem`). Pick once at the start of a session; don't mix styles mid-flow.
...
| `mermaid` | Codex, GitHub, and Markdown viewers with Mermaid support | User is reading in a rendered surface and asks for it explicitly |
...
`--say` reads **spoken turns** aloud through the bundled `scripts/speak.sh` helper (resolve it relative to this skill's directory). Off by default. Only `mock` (Codex = interviewer) and `learn` (Codex = candidate) have a voice role; `--say` is a silent no-op in `postmortem` and `generate`.
